...
def get_movie_info(html,typename):
    ...
    for item in items:
        ...
        info =item['id'] + ',' + typename +','+item['title'] +','+  regions +',' + item['score'] +','+ item['release_date']+','+item['url']+','+item['cover_url']
        print(info)
        save_data(info)
    return ans
...
def main():
   ...
   while(type_idx <  len(type_list)):

      if(interval_idx >= len(interval_list)):
          type_idx += 1
          idx_page = 0
          interval_idx = 0

      type = type_list[type_idx]
      interval_id = interval_list[interval_idx]

      print( typename_list[type_idx] + ',' + interval_id + '---------------------------------------正在爬取：第' + str(idx_page + 1) + '页......')

      params = {"type":type,"interval_id":interval_id,"action":"","start":idx_page * 20,"limit":20}
      html =  get_page_html(url,params)
      if(len(html) <= 2):
            interval_idx += 1
            idx_page = 0
      else:
            get_movie_info(html,typename_list[type_idx])
            idx_page+=1
      rad = randint(1,5)
      time.sleep(rad)
      print( typename_list[type_idx] + ',' + interval_id +'---------------------------------------第' + str(idx_page + 1) + '页完成！')
      print('\n')
